<template>
    <div class="sign-form-container">
        <app-title value="ユーザー再設定" />
        <app-text value="メールアドレスとパスワードを入力し、ユーザーの再設定を完了してください。" />
        <validation-observer v-slot="{ invalid }">
            ...
            <div class="sign-button-container">
                <app-button class="sign-button" @click="signUp" :disabled="invalid">設定する</app-button>
            </div>
        </validation-observer>
    </div>
</template>
...
export default class SignUpForm extends Vue {
    ...
    @AsyncLoadingAndErrorHandle() public async signUp(): Promise<void> {
        try {
            ...
            await hostUserAuthInteractor.signUp(this.newEmail, this.newPassword, { companyID });
            infoStore.addInfo({
                type: 'INFO',
