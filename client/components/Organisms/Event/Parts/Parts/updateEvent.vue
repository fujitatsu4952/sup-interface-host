<template>
    <div class="update_event_modal_back" v-if="isShowCalendar && planTimelyStatus" @click.self="isShowCalendar = !isShowCalendar">
        <div class="update_event_modal">
            <validation-observer v-slot="{ invalid }" class="update_event_modal_validator">
                <app-text :value="`${startTime}~${endTime}のイベント更新`" />
                <div class="update_event_container">
                <div class="update_event_time">
                    <app-text value="開始時刻" />
            ...
            <div class="update_event_plan" v-if="availablePlans && availablePlans.length">
                ...
                <div class="update_event_stock" v-if="availablePlans && availablePlans.length">
                    <app-text value="在庫数" />
                    <app-input-number rule="required" v-model="planTimelyStatus.stockNum" />
                </div>
                <div class="update_event_insale">
                    <app-text value="販売する" />
...
import { KeyValueObject, PlanTimelyStatus, DailyStatusUpdateObject, Scalars } from 'sup_abr';
...
export default class updateEvent extends Vue {
    ...
    @AsyncLoadingAndErrorHandle() public async register() {
        const dailyStatusUpdateObjects:DailyStatusUpdateObject[] = [];
        dailyStatusUpdateObjects.push({
            resourceID: this.planTimelyStatus.resourceID,
            planID: this.planTimelyStatus.planID,
            dateTimeStart: this.planTimelyStatus.dateTimeStart,
            dateTimeEnd: this.planTimelyStatus.dateTimeEnd,
            isAvailable: this.planTimelyStatus.isAvailable,
            availableNum: this.planTimelyStatus.availableNum,
            stockNum: this.planTimelyStatus.stockNum})
        await planTimelyStatusInteractor.updatePlanTimelyStatusMast(this.$route.params.shopID, dailyStatusUpdateObjects, undefined);
